...
void main()
{
	...
	// Only calculate spotlight if valid (prevents blue flashing from uninitialized uniforms)
	vec3 spotlightResult = vec3(0.0f, 0.0f, 0.0f);
	if (length(spotlight.position) > 0.01f || length(spotlight.direction) > 0.01f)
	{
		spotlightResult = CalculateSpotlight(spotlight, fragNormal, viewDir);
	}
